...
In-video quizzes, which are commonly found in courses on platforms such as Coursera, are questions that users are asked to answer upon reaching a certain point in the video, as shown in \autoref{fig:coursera}. In-video quizzes are auto-graded; the majority we observe in courses like the Machine Learning course on Coursera are either multiple-choice or multiple-checkbox, though there also exist a few numeric-response quizzes.
...
\section{Aggregate Statistics of Seek Chains}
...
\section{Visualizing Viewing Behavior in Individual Lectures}
